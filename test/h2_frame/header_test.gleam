...
pub fn parse_header_with_flags_test() {
  ...
  |> should.equal(
    Ok(
      #(
        FrameHeader(length: 0, frame_type: header.Ping, flags: 1, stream_id: 0),
        <<>>,
      ),
    ),
  )
...
pub fn parse_header_with_stream_id_test() {
  ...
  |> should.equal(
    Ok(
      #(
        FrameHeader(length: 0, frame_type: header.Data, flags: 0, stream_id: 5),
        <<>>,
      ),
    ),
  )
...
pub fn parse_header_with_trailing_payload_test() {
  ...
  |> should.equal(
    Ok(
      #(
        FrameHeader(length: 8, frame_type: header.Ping, flags: 0, stream_id: 0),
        <<0, 0, 0, 0, 0, 0, 0, 0>>,
      ),
    ),
  )
...
pub fn encode_header_with_all_fields_test() {
  let header =
    FrameHeader(length: 256, frame_type: header.Headers, flags: 5, stream_id: 7)
  header.encode_header(header)
